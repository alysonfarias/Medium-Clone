      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 p-2'>
        {posts.map((post) => (
          <Link key={post._id} href={`/post/${post.slug.current}`}>
            <div className='border shadow-lg rounded-lg group overflow-hidden cursor-pointer'>
              <img className='h-60 w-full object-cover 
              group-hover:scale-105 transition-transform durantion-200 ease-in-out' src={urlFor(post.mainImage).url()!}/>
